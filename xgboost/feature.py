...
t1['total_sales'] = 1
t1.to_csv('merchant3_fenzu1.csv',index=None)
...
fenzu1 = pd.read_csv('fenzu1.csv')
fenzu1.merchant_id=fenzu1['merchant_id'].apply(str)
fenzu1.head()
fenzu1.dtypes
...
fenzu1 = fenzu1.groupby('merchant_id')["total_sales"].agg('sum').reset_index()
fenzu1.head()
...
fenzu1 = pd.read_csv('fenzu1.csv')
fz1 = fenzu1.groupby('merchant_id')["total_sales"].agg('sum').reset_index()
fz1.head(5)
fenzu1.shape
...
# 显示使用了优惠券消费的商品，正样本
tm['merchant_id'] = 1
#tm['merchant_id']= merchant3[(merchant3.date!='null')&(merchant3.coupon_id!='null')]['merchant_id']
tm['sales_use_coupon'] = 1
#print(t2.reset_index())
#print(merchant3.dtypes)
#t2.head(20)
tn = tm['sales_use_coupon'].groupby(tm['merchant_id'])
tn.count()
#t2 = t2.groupby('merchant_id').agg('sum')
#t2.head()
